...
Entwurfsprinzipien haben jedoch die Eigenheit, dass sie nicht so leicht einzuhalten sind wie konkrete Regeln.
...
Bei einem Datenstrom, der einen eigenen Datentyp beinhaltet, kann es helfen, den Datentyp innerhalb des Flow Design Diagrammes zu definieren.
...
Optionale Outputs können genau wie Datenströme in den meisten Programmiersprachen nicht über ein Rückgabewert einer
...
Falls im Output mehrerer Funktionseinheiten in einen Datenstrom zusammenlaufen
...
Kommunikation zwischen Operationen. Mit Daten sind hauptsächlich nicht-persistente
Daten gemeint, die als Stream durch die Operationen fließen. Daten in Form von Objekten und primitive Datentypen, die von den
...
Das \enquote{Verdrahten} von Datenflüssen übernehmen jedoch die Integrationen (was auf der Abbildung 5.1 leider nicht so deutlich herauskommt). 
...
somit würden solche Aufrufe dann zu der Gruppe API zählen.

Die IODA Architektur besagt, dass API-Aufrufe sich nur innerhalb von Operationen befinden
dürfen, damit diese Informationen gekapselt und die Integrationen frei von API-spezifischem Wissen bleiben.
...
\begin{aquote}{Ralf Westphal \cite[S. 80]{schummelzettel}}
	\enquote{Ein Producer kennt seinen Consumer nicht. Ein Consumer kennt seinen Producer
...
\blankfootnote{LINQ besteht aus ca. 150 Methoden.
	Eine (nicht vollständige Liste) befindet sich hier:\cite{MICROSOFT} } 
...
Üblicherweise entstehen mehrere Output-Wege, sobald eine
