...
namespace Dexel.Model.Manager
{
    ...
    public static class DataStreamManager
    {
        ...
        public static DataStreamDefinition GetDSDFromModel(Guid id, List<FunctionUnit> functionUnits)
        {
            var inputDSDs = functionUnits.Select(sc => sc.InputStreams.FirstOrDefault(dsd => dsd.ID == id))
            ...
            var outputDSDs = functionUnits.Select(sc => sc.OutputStreams.FirstOrDefault(dsd => dsd.ID == id))
                .Where(x => x != null).ToList();
            if (outputDSDs.Any())
            {
